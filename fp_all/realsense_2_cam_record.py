...
def record_a_demo_fp_only(scene_name:str = "", depth_max_dist_meters=2, seq:list[str]=['gripper']):
    ...
    gripper_camera_id = "150622071964"

    pipeline_side = rs.pipeline()
    pipeline_grip = rs.pipeline()

    config_side = rs.config()
    config_grip = rs.config()
    
    config_side.enable_device(side_camera_id)
    config_grip.enable_device(gripper_camera_id)

    config_side.enable_stream(rs.stream.depth, 640, 480, rs.format.z16, 30)
    config_side.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30)
    
    config_grip.enable_stream(rs.stream.color, 640, 480, rs.format.bgr8, 30)

    # Start streaming
    profile_side = pipeline_side.start(config_side)
    profile_grip = pipeline_grip.start(config_grip)

    # Getting the depth sensor's depth scale (see rs-align example for explanation)
    depth_sensor = profile_side.get_device().first_depth_sensor()
    ...
    subfolder_grip = os.path.join(script_dir, "out", t, "grip_cam")


    os.makedirs(subfolder_depth, exist_ok=True)
    os.makedirs(subfolder_rgb, exist_ok=True)
    os.makedirs(subfolder_grip, exist_ok=True)
    ...
    try:
        while True:
            ...
            frames_grip = pipeline_grip.wait_for_frames()

            # frames.get_depth_frame() is a 640x360 depth image

            # Align the depth frame to color frame
            aligned_frames = align.process(frames_side)
            ...
            grip_image = np.asanyarray(frames_grip.get_color_frame().get_data())
            ...
            if RecordStream:
                ...
                image_path_grip = os.path.join(subfolder_grip, f"{framename:06d}.png")
                ...
                cv2.imwrite(image_path_grip, grip_image)
...
if __name__ == "__main__":
    ...
    # except:
    #     scene_name = ""
    # record_a_demo_fp_only(scene_name=scene_name, seq=['gripper', 'box2'])

    # record multiple scenes
    for i in range(36, 37):
        print(f"start to record scene {i} ...")
        scene_name = "side-gripper-cam/" + str(i)
        record_a_demo_fp_only(scene_name=scene_name, seq=['gripper', 'box2'])